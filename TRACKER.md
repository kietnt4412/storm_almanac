...
- **When an item closes, move it to the archive rather than striking it through
  ...
  **Still well over: 747 on 2026-09-11 → 742 on 2026-09-12 (eighteenth) → 746 on
  ...
  **and still ended four lines up, because it closed a phase, opened a deviation and
...
|------|-------|-----------------------|
| Backend build | **Green** | **324 tests**, 0 failed, 0 skipped locally with snapshots present. 16 of them skip on CI (the snapshot-gated three). `:app:test` depends on `:app:bootJar` — `DeployableJarTest` reads the artifact. Test tasks set `api.version=1.44` — [E2](#e2--docker-engine-29-refuses-testcontainers-api-version) |
| CI workflow | **Green as of the eighteenth session; this session's tree is not on a runner yet** | Last green was run `34691539392`; see *Status* for the remote, checked rather than trusted. **16 skipped and they are exactly the three snapshot-gated classes** — `RealUpstreamPlanTest` 8, `CommunityBenchmarkTest` 5, `RealUpstreamPatchTest` 3. A pass there would mean a snapshot had been committed by accident. Action deprecations pending — **N5** |
