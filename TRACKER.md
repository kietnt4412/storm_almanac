...
- **The remote, last checked 2026-09-25 (forty-second) — re-check it, do not trust it, and check three things:**
  ...
  **Neon holds sequence 11** since 11:32Z on 2026-09-25, read back as *no changes*. Stale on eight consecutive checks, so re-check it rather than read it. B6 means a commit on `dev` with no PR open is still built.
...
> **Resume here (2026-09-25, forty-second):** **D5's rehearsal is under way**; every stall is a line below. **Open:** Ultima
> Awaken, reported to cost what the leader unlock does — which contradicts the bundle's 2026-09-24 note of no cost on its screen; asked, not yet recorded. Seen, not yet a stall: stage and *craft and buy* rows on the plan page show raw ids.
