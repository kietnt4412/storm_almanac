...
- Last updated: **2026-09-26** (forty-sixth session)
...
- **PGR is read first-hand** ([the note](docs/game-facts/punishing-gray-raven-research-disclosure.md)); [its bundle](data/bundles/punishing-gray-raven-steering-by-light.json)
  is **sequence 13** (Global 4.8.0 "Anchored in Faith") — three constructs on one S-rank ladder, one weekly ladder, a 05:00 UTC
  reset, a pull price, **since sequence 11 the game's own words for ranks and skill sections** (ADR 0032, S4), and **since 13 words for its own keys** — the Cage's score, item categories, entity kinds (ADR 0033, S8). Every sequence
...
- **Fourteen decisions are live, and each is its own account — read the ADR, never a summary.**
  ...
  [0031](docs/adr/0031-a-shared-upgrade-path-is-written-once-and-expanded-in-the-file.md) shared ladders ·
  [0033](docs/adr/0033-a-key-the-bundle-invents-may-be-given-a-word.md) words for keys. **0022's trigger is
  live:** `reach` becomes player state the moment a reader answers twice, which since 2026-09-21 means *a second
  device*. **Three loose ends sit outside all fourteen:** the **Themed Construct archetype is expressible and not
...
- **The remote, last checked 2026-09-26 (forty-sixth) — re-check it, do not trust it, and check three things:**
  ...
  **`deploy` never ran** and nothing showed it. [PR #47](https://github.com/kietnt4412/storm_almanac/pull/47)
  fixed it; #48 (D5, S1), #49 (S2, S3), #50 (S4, V16), #51 (sequence 12), #52 (S5), #53 (docs), [#54](https://github.com/kietnt4412/storm_almanac/pull/54) (S7, S8, `V17`) and #55 (docs) followed, and **production reports `3bfa8a9`**, #55's merge, its `main` run `36238224339` green. **S9 and S10 are on `dev` and in no PR yet.**
  **Neon holds sequence 13** since 11:06Z on 2026-09-26, read back as *no changes*, and the live `/measures` route names the Cage. Stale on eight of nine checks, so re-check it rather than read it. B6 means a commit on `dev` with no PR open is still built.
...
> **Resume here (2026-09-26, forty-sixth):** **S9 and S10 are done on `dev`, in no PR yet** — check whether one merged. **S6 waits on a reading**; then the stall list is the *smaller* line, which is the maintainer's call: fix it or cut it to C2. Empty list → D5's trigger, strangers. **Open:** N42 below.

### D5's rehearsal — each stall, fixed or cut

**S1–S5 closed 2026-09-25/26, S7–S10 2026-09-26** ([archive](docs/history/tracker-archive.md#completed-next-actions)). **The third run** (forty-fifth, fresh account `rehearsal-3`, local, dev sign-in) went home → profile → inventory → Lucia on the roster → **one goal row, three targets** → a plan (180 Serum, 0.8 days, every line named): **it completes; the two things that read wrong, S9 and S10, are fixed.**
...
|---|---|---|
| 2026-09-26 | forty-sixth | Production on `3bfa8a9` (#55). **S9**: a purchase says its totals, "Buy 514,800 Cogs for 429 Simulation Score · 429 × 1,200 for 1", in a new wire field so a page and server a deploy apart cannot disagree. **S10**: zero shadow prices become one sentence saying why. Driven at 375 and 1280 |
