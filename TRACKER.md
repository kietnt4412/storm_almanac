...
- Last updated: **2026-09-24** (thirty-ninth session)
...
- [ ] **N41 — Put Selena: Pianissimo and Karenina: Effulgence on the S-rank ladder** (Q6).
      ...
      **Left is words, per construct:** eight levelled skill names, the SS passive's name, the
      ...
      is written, not published** — Lacrimosa's skill list fixed (−4/+8 in `preview`, 121
      facts); publishing it into the local volume *and* Neon is the maintainer's approval.
      Production goals were not checked for an `astral-armament-*` state.
