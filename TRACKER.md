...
- Last updated: **2026-09-05** (fourth session)
- Current phase: **Phase 1 — Game data foundation**, opened 2026-09-05 with the
  ...
  `gamedata-cli` all landed the same day. What is left of the phase is serving
  it over HTTP — see **N7**. Phase 0 is closed by exception (deploy deferred
  by D1).
...
**What exists:** the repo skeleton, the domain model, a green backend build, an
application that runs and serves `/api/health`, and — as of this session — **a
game data pipeline that works end to end**. A bundle is parsed, validated,
written as a draft, previewed as a diff, approved, and read back as the same
records that went in. `gamedata-cli` drives all of it from the command line
without touching backend code, which is the claim the plan makes about
onboarding a title.

The load-bearing sentence: **the schema has now held data and given it back
unchanged.** Everything before this session proved the migration applies and its
constraints reject what they claim to; none of that is the same as a round trip.
Doing it found exactly the class of defect it existed to find — a version could
not be deleted at all — see the session log and `V3`.
...
| Area | State | Notes |
|------|-------|-------|
| Backend build | **Green** | `./gradlew build` — 65 tests, `storm-almanac.jar` |
...
| Module ports | Partly implemented | `GameDefinitionRepository` now has one. `Optimizer`, `SolveCoordinator`, `DropReportStore`, `BannerEngine` are still interfaces with nothing behind them |
...
- **CI has not run on this session's work.** It was green on `662d999`
  (runs `33965175789` and `33965237825`) and this session added three
  Testcontainers-backed test classes and a third Postgres container to the
  build — the two new gamedata classes share one, `ApplicationBootTest` and
  `GameDataSchemaTest` keep theirs. Locally the whole suite is 22 seconds. On
  the runner it is unproven. **First action next session: push, then check.**
  The rule stands: a pipeline is proven for the tree it ran on and no other.
...
- **The API still serves one endpoint.** `/api/health` and nothing else. The
  gamedata pipeline is exercised by the CLI and by tests; no HTTP route reaches
  it, and `SecurityConfig` denies everything that is not explicitly public. That
  is the gap between "Phase 1 works" and Phase 1's exit criterion, which says
  *your API* answers the two questions — see **N7**.
...
- [ ] **N7 — Serve it. Phase 1's exit criterion is an API answer, not a test.**
      The data is in the schema and the questions are answerable — see
      `answersTheProgressionQuestion` and `answersTheCatalogQuestion`, which ask
      them of the repository. The criterion says *your API* answers them, and
      there is still exactly one endpoint in this application. Needed:
      a read model in `api` over `GameDefinitionRepository`, endpoints for
      catalog entity, upgrade costs and the version list, and the diff report
      rendered. `SecurityConfig` denies everything that is not `/api/health`, so
      each new route has to be added to the public matcher deliberately —
      that is working as designed, not an obstacle to route around.
...
- [ ] **Phase 1 · Game data foundation** — 2.5 weeks — *in progress, opened
      ...
      **Landed:** the canonical schema (`V2`, 28 tables, plus `V3` making a
      ...
      publishing with a draft/approve split, the three-axis patch diff, and
      `gamedata-cli`. **The schema holds data and returns it unchanged** —
      `GameDataIngestTest.roundTripsWithoutLoss` asserts record equality across
      the whole graph.
      **Not met yet, and both halves matter:**
      - *the API answers* — it does not. Nothing but `/api/health` is served;
        the two questions are answered by the repository, in tests. **N7.**
      - *tests over real patch data* — the patch is real in shape and synthetic
        in origin. `proving-ground` 1.0 → 1.1 changes something on each axis and
        the diff reports it, but no upstream bundle has been ingested. Gated on
        Q3/F1/F2. **N8.**
