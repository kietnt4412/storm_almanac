...
- **PGR is read first-hand** ([the note](docs/game-facts/punishing-gray-raven-research-disclosure.md)); [its bundle](data/bundles/punishing-gray-raven-steering-by-light.json)
  is **sequence 14** (Global 4.8.0 "Anchored in Faith"; 14 only turned every grade's `*` into ★) — three constructs on one S-rank ladder, one weekly ladder, a 05:00 UTC
...
- **The remote, last checked 2026-09-26 (forty-sixth) — re-check it, do not trust it, and check three things:**
  ...
  **`deploy` never ran** and nothing showed it. [PR #47](https://github.com/kietnt4412/storm_almanac/pull/47)
  fixed it; #48 (D5, S1), #49 (S2, S3), #50 (S4, V16), #51 (sequence 12), #52 (S5), #53 (docs), [#54](https://github.com/kietnt4412/storm_almanac/pull/54) (S7, S8, `V17`) and #55 (docs) followed, and **production reports `3bfa8a9`**, #55's merge, its `main` run `36238224339` green. **[PR #56](https://github.com/kietnt4412/storm_almanac/pull/56) (S9, S10, the smaller hesitations) is open**, CI running when last looked.
  **Neon holds sequence 14** since 12:06Z on 2026-09-26, read back as *no changes*, and the live `/items` route serves ★ — ahead of #56's code, which nothing in it depends on. Stale on eight of nine checks, so re-check it rather than read it. B6 means a commit on `dev` with no PR open is still built.
...
> **Resume here (2026-09-26, forty-sixth):** **[#56](https://github.com/kietnt4412/storm_almanac/pull/56) is open** (S9, S10, the smaller hesitations) — check it merged and deployed; sequence 14 is already on Neon. **S6 is the last stall, and it waits on a reading** (Skill Points and Cogs per level 4 → 18). Once it closes the list is empty → D5's trigger: strangers, where players already are. **Open:** N42 below.

### D5's rehearsal — each stall, fixed or cut

**S1–S5 closed 2026-09-25/26, S7–S10 and the smaller hesitations 2026-09-26** ([archive](docs/history/tracker-archive.md#completed-next-actions)). **The third run** (forty-fifth, fresh account `rehearsal-3`, local, dev sign-in) went home → profile → inventory → Lucia on the roster → **one goal row, three targets** → a plan (180 Serum, 0.8 days, every line named): **it completes; the two things that read wrong, S9 and S10, are fixed.**
