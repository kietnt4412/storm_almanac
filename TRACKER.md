...
- **The remote, last checked 2026-09-25 (forty-second) — re-check it, do not trust it, and check three things:**
  ...
  **`deploy` never ran** and nothing showed it. [PR #47](https://github.com/kietnt4412/storm_almanac/pull/47)
  fixed it; #48 (D5, S1) and #49 (S2, S3) followed, and **production reports `02fbd51`**, #49's merge. `dev` carries
  S4 (ADR 0032, V16) in the next PR, and **Neon still holds sequence 10** until it is published there. Stale on eight consecutive checks, so re-check it rather than read it. B6 means a commit on `dev` with no PR open is still built.
...
> **Resume here (2026-09-25, forty-second):** **D5's rehearsal is under way**; every stall is a line below. **Open:** publish
> sequence 11 to Neon once S4's PR has deployed (needs the maintainer's OK), and Ultima Awaken's costs, which the maintainer will read. Seen, not yet a stall: stage and *craft and buy* rows on the plan page show raw ids.
